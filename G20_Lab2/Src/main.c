...
int main(void)
{
  ...
  while (1)
  {
		...
		/* Outputs the sawtooth waveform */
		
		if(data == 4096){
			//the scope reaches its max value
			data = 0;
		}
		//increment the data and write it to output pin
		...
		//start the DAC CHANNEL.
		HAL_DAC_Start(&hdac1, DAC_CHANNEL_2);
		//Generate a triangle wave with max amplitude of 4095.
		HAL_DACEx_TriangleWaveGenerate(&hdac1, DAC_CHANNEL_2, 4095);
		
		
		/* Outputs a rectangular waveform */
		/*
		if(data == 4096){
			decrementing = 1;
		} else if (data == 0){
